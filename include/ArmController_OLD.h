				...
				const std::string &_name)
{
	...
	configure_drivers(local_port_name, remote_port_name, _name+" arm", 10);	// Should be 10 joints in the iCub arm
}

/******************** Get the error between desired pose and current pose ********************/
yarp::sig::Vector ArmController::get_pose_error(const yarp::sig::Matrix &desired)
{
	...
	yarp::sig::Matrix actual = getH();						// Get the current end-effector pose as SE3
