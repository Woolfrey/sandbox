...
class QuinticRot : public Quintic
{
	public:
		...
		QuinticRot(const iDynTree::Rotation &startPoint,
...
QuinticRot::QuinticRot(const iDynTree::Rotation &startPoint,
			const iDynTree::Rotation &endPoint,
			const double &startTime,
			const double &endTime)
			: R0(startPoint)
...
/******************** Get the desired state for the orientation ********************/
bool QuinticRot::get_state(iDynTree::Rotation &rot,
			iDynTree::GeomVector3 &vel,
			iDynTree::GeomVector3 &acc,
			const double &time)
{
	...
	{
		std::cerr << "[ERROR][QUINTICROT] get_state() : Could not obtain the state for the given time." << std::endl;
		rot = this->R0;
		for(int i = 0; i < 3; i++)
		{
			vel[i] = 0;
