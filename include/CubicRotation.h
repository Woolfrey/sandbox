	...
	else
	{
		...
		{	
			...
			for(int i = 0; i < this->m; i++)
			{
				...
				a[i][0] = -2*(points[1][i] - points[0][i])/pow(dt,3);    // This term determines the shape of the position profile
				b[i][0] =  3*(points[1][i] - points[0][i])/pow(dt,2);    // This term determines the shape of the velocity profile
