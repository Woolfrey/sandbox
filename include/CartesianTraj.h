#ifndef CARTESIANTRAJ_H_
#define CARTESIANTRAJ_H_
...
#include <QuinticRot.h>									// Also includes Quintic.h
...
class CartesianTraj
{
	public:
		...
		CartesianTraj(const iDynTree::Transform &startPose,
	...
	private:
		...
		QuinticRot rotTraj;							// Rotation trajectory
	
};											// Semicolon needed after class declaration

/******************** Constructor for 2 waypoints **********/
CartesianTraj::CartesianTraj(const iDynTree::Transform &startPose,
				const iDynTree::Transform &endPose,
				const double &startTime,
				const double &endTime)
				: T0(startPose)
{
	...
	this->rotTraj = QuinticRot(startPose.getRotation(), endPose.getRotation(), startTime, endTime);
}

/******************** Get the desired state for the given time **********/
bool CartesianTraj::get_state(iDynTree::Transform &pose,
				iDynTree::Twist &vel,
				iDynTree::SpatialAcc &acc,
				const double &time)
{
	...
	{
		std::cerr << "[WARNING][CARTESIANTRAJ] get_state() : There was a problem obtaining the desired state." << std::endl;
