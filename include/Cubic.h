...
class Cubic
{
	...
	private:
		bool isNotValid = false;						// Won't do any interpolation if this is true
		...
		void compute_coefficients(const iDynTree::MatrixDynSize &C,
					const std::vector<iDynTree::VectorDynSize> &p);
	...
	, t(times)
{
	...
	else
	{
		...
		if(!this->isNotValid)								// No problems with construction
		{
			...
			{	
				...
				for(int i = 0; i < this->m; i++)
				{
					...
					a[i][0] = -2*(points[1][i] - points[0][i])/pow(dt,3);
					b[i][0] =  3*(points[1][i] - points[0][i])/pow(dt,2);	//
...
//					sdd = C*s;							// Compute the coefficients
					...
					for(int j = 0; j < this->n-1; j++)				// Compute coefficients for the n-1 splines
					{
						...
						this->b[i][j] = sdd[j]/2;				
						if(j == 0)		this->c[i][j] = 0;		// Starting velocity of zero for first spline
						else if(j < this->n-1)	this->c[i][j] = ds/dt - dt*(sdd[j+1] - 2*sdd[j])/6;
						else			this->c[i][j] = dt*(sdd[j+1] + sdd[j])/2;
						this->d[i][j] = s[j];					// d dictates the start position of the spline
					}
				}
			}
		}
